...
def build_image(release, version, build, fix):
    DOCKER_TAG_CUSTOM = f"docker-{release}:{release}-{version}.{build}.{fix}"
    ...
    for block in blocks:
        ...
        if release is not None and version is not None and build is not None and fix is not None:
            ...
            if block not in old_state:
                print(f"Building Docker image for {release} with version={version}, build={build}, fix={fix}")
                build_image(release, version, build, fix)
            else:
